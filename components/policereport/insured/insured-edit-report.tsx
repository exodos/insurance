...
const UpdateInsuredPoliceReport = gql`
  mutation UpdateInsuredPoliceReport(
    $updateInsuredPoliceReportId: String!
    $input: InsuredPoliceReportUpdateInput!
  ) {
    updateInsuredPoliceReport(id: $updateInsuredPoliceReportId, input: $input) {
      id
      incidentNumber
      victimDriverName
      victimLicenceNumber
...
const InsuredPoliceReportEditModal = ({ policeReport, href }) => {
  const notificationCtx = useContext(NotificationContext);
  const [open, setOpen] = useState<boolean>(true);

  const [updatepoliceReport, { data, error, loading }] = useMutation(
    UpdateInsuredPoliceReport
  );
  if (error) {
    console.log(error);
  }
  ...
  const initialValues = {
    ...
    // branchName: policeReport.branch.branchName,
    // responsibleVehiclePlateCode: policeReport.victimLicenceNumber,
    // responsibleVehiclePlateRegion: policeReport.victimLicenceNumber,
    // responsibleVehiclePlateNumber: policeReport.victimLicenceNumber,
  };
  ...
  const onSubmit = async (values: any) => {
    ...
    await updatepoliceReport({
      variables: {
        updateInsuredPoliceReportId: policeReport.id,
        input,
      },
      onError: (error) => {
        setOpen(false);
...
export default InsuredPoliceReportEditModal;
