...
const CreateCertificateBranch = gql`
  mutation CreateCertificateBranch(
    $plateNumber: String!
    $input: CertificateCreateInput!
  ) {
    createCertificateBranch(plateNumber: $plateNumber, input: $input) {
      insureds {
        mobileNumber
      }
      vehicles {
...
const BranchAddCertificateModal = ({ regionCode, codeList, href }) => {
  ...
  const [createCertificateBranch, { data, error, loading }] = useMutation(
    CreateCertificateBranch
  );
  if (error) {
    console.log(error);
  }
...
export default BranchAddCertificateModal;
