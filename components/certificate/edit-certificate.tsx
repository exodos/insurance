...
const UpdateCertificate = gql`
  mutation UpdateCertificate(
    $updateCertificateId: String!
    $input: CertificateUpdateInput!
  ) {
    updateCertificate(id: $updateCertificateId, input: $input) {
      id
      updatedAt
      policies {
        policyNumber
...
const EditCertificateModal = ({ certificate, href }) => {
  const notificationCtx = useContext(NotificationContext);
  const [open, setOpen] = useState<boolean>(true);

  const [updateCertificate, { data, error, loading }] =
    useMutation(UpdateCertificate);
  ...
  const onSubmit = async (values: any) => {
    ...
    await updateCertificate({
      variables: {
        updateCertificateId: certificate.id,
        input,
      },
      onError: (error) => {
        setOpen(false);
      ...
      onCompleted: (data) => {
        ...
        notificationCtx.showNotification({
          title: "Success!",
          message: "Successfully Updated Certificate Data",
          status: "success",
        });
      },
      update: (cache, { data }) => {
