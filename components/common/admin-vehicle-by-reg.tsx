...
const FeedVehicleByRegNumber = gql`
  query FeedVehicleByRegNumber($regNumber: String!) {
    feedVehicleByRegNumber(regNumber: $regNumber) {
      vehicles {
        id
        plateNumber
        engineNumber
...
const AdminVehicleByRegNumber = ({ branchId, path }) => {
  ...
  const [
    vehicleData,
    {
      loading: feedVehicleByRegNumberLoading,
      error: feedVehicleByRegNumberError,
      data: feedVehicleByRegNumberData,
    },
  ] = useLazyQuery(FeedVehicleByRegNumber);
      ...
      <div className="max-w-[90%] mx-auto px-4 sm:px-6 lg:px-8 py-4 bg-white mt-20 rounded-3xl">
        <div className="space-y-8 sm:space-y-5">
          ...
          <div className="px-10 ml-10">
            {feedVehicleByRegNumberError && (
              <p className="mt-2 max-w-2xl text-lg font-medium text-eRed">
                {feedVehicleByRegNumberError.message}
              </p>
            )}
          </div>
          <div className="max-w-[80%] mx-auto px-4">
            {feedVehicleByRegNumberData?.feedVehicleByRegNumber?.vehicles
              ?.length <= 0 && (
                    ...
                    <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                      <table className="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                          {feedVehicleByRegNumberData?.feedVehicleByRegNumber
                            ?.vehicles?.length > 0 &&
                            feedVehicleByRegNumberData?.feedVehicleByRegNumber?.vehicles?.map(
                              (item: any, i: any) => (
                                <tr key={item.id}>
...
export default AdminVehicleByRegNumber;
