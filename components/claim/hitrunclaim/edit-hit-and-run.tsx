...
const UpdateClaimHitAndRunDamageEstimate = gql`
  mutation UpdateClaimHitAndRunDamageEstimate(
    $updateClaimHitAndRunDamageEstimateId: String!
    $damageEstimate: Float!
  ) {
    updateClaimHitAndRunDamageEstimate(
      id: $updateClaimHitAndRunDamageEstimateId
...
const EditHitAndRunClaim = ({ hitAndRunClaims, href }) => {
  const notificationCtx = useContext(NotificationContext);
  const [open, setOpen] = useState<boolean>(true);

  const [updateClaimHitAndRun, { data, error, loading }] = useMutation(
    UpdateClaimHitAndRunDamageEstimate
  );
  if (error) {
    console.log(error);
  ...
  const initialValues = {
    damageEstimate: hitAndRunClaims.damageEstimate,
  };
  ...
  const onSubmit = async (values: any) => {
    ...
    await updateClaimHitAndRun({
      variables: {
        updateClaimHitAndRunDamageEstimateId: hitAndRunClaims.id,
        damageEstimate,
      },
      onError: (error) => {
        setOpen(false);
      ...
      onCompleted: (data) => {
        ...
        notificationCtx.showNotification({
          title: "Success!",
          message: "Successfully Updated Hit And Run Claim Data",
          status: "success",
        });
      },
      update: (cache, { data }) => {
...
export default EditHitAndRunClaim;
