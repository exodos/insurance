...
const UpdateInsuranceFromImport = gql`
  mutation UpdateInsuranceFromImport(
    $updateInsuranceFromImportId: String!
    $input: insuranceImportUpdateInput!
  ) {
    updateInsuranceFromImport(id: $updateInsuranceFromImportId, input: $input) {
      id
    }
  }
`;
...
const ImportInsurance = ({
  ...
  const insuredId = router.query.insured;
  console.log(insuredId);

  const [updateInsuranceFromImport, { data, error, loading }] = useMutation(
    UpdateInsuranceFromImport
  );
  ...
  const handleSubmit = async (values: any) => {
    const input = {
      ...
      vehicles: [...parsedData],
      // vehicles: JSON.stringify(parsedData),
    };

    console.log(input);

    await updateInsuranceFromImport({
      variables: {
        updateInsuranceFromImportId: insuredId,
        input,
      },
      onError: (error) => {
