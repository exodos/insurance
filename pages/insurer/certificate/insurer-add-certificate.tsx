...
const VehicleInsurerByPlateNumber = gql`
  query VehicleInsurerByPlateNumber($plateNumber: String!, $orgId: String!) {
    vehicleInsurerByPlateNumber(plateNumber: $plateNumber, orgId: $orgId) {
      id
      plateNumber
      engineNumber
      chassisNumber
...
const InsurerAddCertificate = ({
  ...
  const [
    vehicleData,
    {
      loading: vehicleInsurerByPlateNumberLoading,
      error: vehicleInsurerByPlateNumberError,
      data: vehicleInsurerByPlateNumberData,
    },
  ] = useLazyQuery(VehicleInsurerByPlateNumber);
  ...
  return (
    <>
      ...
      <div className="max-w-[90%] mx-auto px-4 sm:px-6 lg:px-8 py-4 bg-white mt-20 rounded-3xl">
        <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
          ...
          <div className="px-10 ml-10">
            {vehicleInsurerByPlateNumberError && (
              <p className="mt-2 max-w-2xl text-lg font-medium text-eRed">
                {vehicleInsurerByPlateNumberError.message}
              </p>
            )}
          </div>
          <div className="px-10 ml-10">
            {vehicleInsurerByPlateNumberData?.vehicleInsurerByPlateNumber ===
              null && (
              <p className="mt-2 max-w-2xl text-lg font-medium text-eRed">
                    ...
                    <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                      <table className="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                          <tr
                            key={
                              vehicleInsurerByPlateNumberData
                                ?.vehicleInsurerByPlateNumber?.id
                            }
                          >
                            <td className="whitespace-normal py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.plateNumber
                              }
                            </td>
                            <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.engineNumber
                              }
                            </td>
                            <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.chassisNumber
                              }
                            </td>
                            <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.vehicleType
                              }
                            </td>
                            <td className="whitespace-normal py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.vehicleSubType
                              }
                            </td>
                            <td className="whitespace-normal py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.vehicleDetails
                              }
                            </td>
                            <td className="whitespace-normal text-ellipsis overflow-hidden ... py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.vehicleUsage
                              }
                            </td>
                            <td className="whitespace-normal py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.vehicleCategory
                              }
                            </td>
                            <td className="whitespace-normal py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.passengerNumber
                              }
                            </td>
                            <td className="whitespace-nowrap py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.premiumTarif
                              }
                            </td>
                            <td className="whitespace-normal py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                              {
                                vehicleInsurerByPlateNumberData
                                  ?.vehicleInsurerByPlateNumber?.isInsured
                              }
                            </td>
      ...
      {showCreateModal ? (
        <AddCertificateModal
          vehicle={vehicleInsurerByPlateNumberData?.vehicleInsurerByPlateNumber}
          href={path}
        />
      ) : null}
    </>
